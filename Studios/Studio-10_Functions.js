...
function reverseCharacters(getsReversed){
    return getsReversed.split('').reverse().join('');
}
...
function reverseCharacters(getsReversed){
    if(typeof getsReversed == 'number'){
      //turns num to string, then array, reverses it, back to string, then back to num (base 10) using parseInt
      return(parseInt(getsReversed.toString().split('').reverse().join(''),10));
    }
    return getsReversed.split('').reverse().join('');
}
...
//Spliting up the functionality of a for loop to be outside of a function
let testArr = ['asdf', 'jklj;', 4321];
let i=-1;
function test(arr) {
  i++
  return(reverseCharacters(arr));
}

console.log(test('afjhgfjfsdf'));
console.log(i);

while(i<3){
  console.log(test(testArr[i]));
}
// let test = 7654;

// test=parseInt(test.toString().split('').reverse().join(''),10);
// // test = test;
// console.log(test);
